...
const question = ref("");
const characterCount = computed(() => `${question.value.length}/100`);
const handleInput = () => {
  if (question.value.length > 100) {
    question.value = question.value.slice(0, 100);
  }
};
...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    const response = await axios.post(
      "https://workflow.quoti.cloud/webhook/orion-form",
      formValues
    );
...
<template>
  <section
    class="flex justify-center bg-form px-6 lg:px-[4rem] xl:px-[10rem] py-20"
  >
    <div
      class="py-16 md:py-0 h-full md:flex md:items-center md:justify-center xl:flex xl:items-center xl:justify-center max-w-[50rem]"
    >
      <div class="flex flex-col">
        <h2
          class="mb-12 text-orion-display-sm text-orion-neutrals-500 font-semibold"
        >
      ...
      <form
        class="flex flex-col flex-wrap gap-[1.81rem] py-14 md:py-8 md:px-8"
        ...
        <div class="form-control w-full md:w-[400px] mb-4">
          <label class="label mb-4">
            <span
              class="label-text text-orion-title-md font-medium text-orion-neutrals-500"
              :class="phone ? '' : ''"
              >Como podemos te ajudar?</span
            >
            <div>
              <p class="text-[0.75rem] text-orion-neutrals-500">
                {{ characterCount }}
              </p>
            </div>
          </label>
          <textarea
            v-model="question"
            @input="handleInput"
            class="bg-orion-secondary-500 border-b-2 border-orion-secondary-300 focus:bg-white text-white focus:text-orion-primary-500 input h-24 rounded-none border-0 placeholder:text-grey focus:outline-none focus:ring-0 focus:border-b-4 focus:border-orion-primary-500 outline-offset-0"
            placeholder="Escreva sua mensagem"
            id="form-doubt"
            :class="'bg-white' ? question.length != 0 : 'bg-transparent'"
          ></textarea>
          <h4 class="text-[0.75rem] text-orion-neutrals-500 cursor-pointer mt-1">
            Ajuda
          </h4>
        </div>
        <div>
